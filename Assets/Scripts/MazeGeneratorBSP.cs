...
public class MazeGeneratorBSP : MonoBehaviour
{
    ...
    public void DrawRooms(SubDungeon subDungeon)
    {
        ...
        if (subDungeon.IAmLeaf())
        {
            for (int i = (int)subDungeon.room.x; i < subDungeon.room.xMax; i++)
            {
                ...
                {
                    GameObject instance = Instantiate(floorTile, new Vector3(i * wallWidth, 0, j * wallWidth), Quaternion.identity) as GameObject;
        ...
        foreach (Rect corridor in subDungeon.corridors)
        {
            for (int i = (int)corridor.x; i < corridor.xMax; i++)
            {
                for (int j = (int)corridor.y; j < corridor.yMax; j++)
                {
                    ...
                    {
                        GameObject instance = Instantiate(corridorTile, new Vector3(i * wallWidth, 0f, j * wallWidth), Quaternion.identity) as GameObject;
                        instance.transform.SetParent(transform);
                        boardPositionsFloor[i, j] = instance;
                    }
                }
